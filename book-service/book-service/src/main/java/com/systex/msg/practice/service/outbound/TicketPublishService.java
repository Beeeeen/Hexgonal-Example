...
@AllArgsConstructor
public class TicketPublishService extends BasePublisherService{

	EventLogRepository eventLogRepository;
	
	
	@TransactionalEventListener//called When transact successfully
	public void handleTicketTrainCreatedEvent(TicketCreatedEvent event) {
	
		try {
			
			EventLog eventLog = eventLogRepository.findByUuid(event.getEventLogUuid());
			
			log.info("[EVENT] send event, data: {}", event);
			
			//Publish message
			AwsSqsClient client = new AwsSqsClient();
			Future<SendMessageResult> future= client.SendMessage(
					"https://sqs.us-west-1.amazonaws.com/430188351668/TicketQueue",eventLog.getBody());
